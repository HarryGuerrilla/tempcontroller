...
// args = ['batch', epoch-milliseconds, temp];
  setup: function(callback) {
    db.serialize(function(){
      if (!exists) {
        ...
        var stmt_temps = "CREATE TABLE ambient_temps (batch TEXT, time INTEGER, temperature FLOAT)";
        console.log(stmt_temps);
        db.run(stmt_temps, function(err){
          if(err){console.log(err);}
        });
  ...
  save: function (args, table) {
    db.run("BEGIN TRANSACTION", function(err) {
    });
    var stmt = "INSERT INTO " + table + " (batch, time, temperature) VALUES (?, ?, ?)";
...
// args = ['batch', startime, endtime]
  get: function (args, table, callback) {
    if(args[2] == -1) {
      args[2] = Date.parse(new Date());
    }
    var stmt = "SELECT * FROM " + table + " WHERE batch = \"" + args[0] + "\" AND (time BETWEEN " + args[1] + " AND " + args[2] + ")";
    console.log(stmt);
  ...
  clear: function(batch) {
    db.serialize(function() {
      ...
      db.run("DELETE FROM targets WHERE batch=\"" + batch + "\"", function(err){});
      db.run("INSERT INTO targets (batch, time, temperature) VALUES ('batch', " +
             (new Date).getTime() + ", 65)", function(err) {});
    });
  },
